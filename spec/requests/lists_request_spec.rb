  ...
  describe 'PATCH #update' do
  	...
  	context '正常なパラメータの場合' do
  		it 'リクエストが成功すること(showページからの更新)' do
  			...
  			params: { list: FactoryBot.attributes_for(:other_list_a, user_id: user.id, genre_id: genre.id), list_id: list.id }
  			expect(response.status).to eq 302
  		end
  	...
  	context '不正なパラメータの場合' do
  		it 'リクエストが成功すること' do
  			patch user_list_url(user_id: user.id, id: list.id),
  			params: { list: FactoryBot.attributes_for(:other_list_a, :invalid, user_id: user.id, genre_id: genre.id), list_id: list.id }
  			expect(response.status).to eq 200
  		end
  		...
  		it 'Titleが更新されないこと' do
  			expect do
  				patch user_list_url(user_id: user.id, id: list.id),
	  			params: { list: FactoryBot.attributes_for(:other_list_a, :invalid, user_id: user.id, genre_id: genre.id), list_id: list.id }
	  		end.to_not change { List.find(list.id).title }
	  	end
  	end
  end
