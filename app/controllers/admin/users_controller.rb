class Admin::UsersController < ApplicationController
    ...
    respond_to do |format|
      ...
      format.csv do
        ...
        send_data render_to_string,
                  filename: "登録会員一覧-#{Time.zone.now.strftime('%Y%m%d')}.csv", type: :csv
      end
    end
  end
  ...
  def chart
    ...
    User.all.each do |user|
      create_user_array << user.created_at.strftime("%Y年%m月%d日").to_s
    end
    ...
    Article.all.each do |article|
      create_article_array << article.created_at.strftime("%Y年%m月%d日").to_s
    end
